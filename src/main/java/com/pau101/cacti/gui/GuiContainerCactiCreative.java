...
public final class GuiContainerCactiCreative extends GuiContainerCreative {
	...
	private void updateTabs(ImmutableList<CreativeTabs> tabs) {
		...
		for (CreativeTabs tab : tabs) {
			...
			if (tab == CreativeTabs.BUILDING_BLOCKS) {
				hasBlockTab = true;
			}
			hasNothing = false;
		}
		if(hasNothing) {
			tabsArray = new CreativeTabs[12];
			isVanillaTabs = hasBlockTab = true;
		}
		if(!isVanillaTabs || !hasBlockTab) {
			for (CreativeTabs tab : tabs) {
				if (tab == CreativeTabs.INVENTORY || tab == CreativeTabs.SEARCH || tab == CreativeTabs.HOTBAR) {
					continue;
				}
				tabsArray[len++] = tab;
			}
		} else {
			tabsArray[len++] = CreativeTabs.BUILDING_BLOCKS;
			tabsArray[len++] = CreativeTabs.DECORATIONS;
			tabsArray[len++] = CreativeTabs.REDSTONE;
			tabsArray[len++] = CreativeTabs.TRANSPORTATION;
			tabsArray[len++] = CreativeTabs.MISC;
			tabsArray[len++] = CreativeTabs.FOOD;
			tabsArray[len++] = CreativeTabs.TOOLS;
			tabsArray[len++] = CreativeTabs.COMBAT;
			tabsArray[len++] = CreativeTabs.BREWING;
		}
		CreativeTabs[] arr = CreativeTabs.CREATIVE_TAB_ARRAY = new CreativeTabs[Math.max(len + 3, 12)];
		arr[CreativeTabs.SEARCH.tabIndex] = CreativeTabs.SEARCH;
		arr[CreativeTabs.INVENTORY.tabIndex] = CreativeTabs.INVENTORY;
		arr[CreativeTabs.HOTBAR.tabIndex] = CreativeTabs.HOTBAR;
		...
		setTabPage(0);
		if(!isVanillaTabs || !hasBlockTab) setCurrentCreativeTab(Iterables.getFirst(tabs, CreativeTabs.INVENTORY));
		else setCurrentCreativeTab(CreativeTabs.BUILDING_BLOCKS);
	}
	...
	private void updateRibbonWidths() {
		for (int i = 0; i < ribbonWidths.length; i++) {
			if (i < currentPageEntries.size()) {
				ribbonWidths[i] = fontRenderer.getStringWidth(currentPageEntries.get(i).getDisplayName());
			} else {
				ribbonWidths[i] = 0;
			}
		}
	...
	private void drawBackground() {
		...
		if (parentCategoryButtonVisible || renderPages) {
			...
			if (parentCategoryButtonVisible) {
				...
				parentCategory.displayString = fitString(mc.fontRenderer, currentCategory.getDisplayName(), parentCategory.width - 6);
			}
			if (renderPages) {
				if (Configurator.displaySide() == DisplaySide.LEFT) {
				...
				font.drawString(str, (pagePrevious.x + pagePrevious.width + pageNext.x) / 2 - w / 2, guiTop + ySize + 7, 0xFFFFFF);
			}
		}
	}
	...
	private void drawScreen(int mouseX, int mouseY) {
		...
		if (entry != null) {
			...
			if (!name.equals(shown)) {
				drawHoveringText(entry.getDisplayName(), mouseX, mouseY);
			}
		}
		GlStateManager.disableLighting();
	}
		...
		for (ModContainer mod : creativeTabs.keySet()) {
			...
			if (tabs.size() == 1) {
				...
				if (checkAdjustTab && currentTabGroup.getId().equals(mod.getModId())) {
					...
					// skip over hotbar and search
					if (groupedMods.size() > 3) {
						tabId+=2;
					}
					// skip over inventory
					if (groupedMods.size() > 8) {
						tabId++;
					}
